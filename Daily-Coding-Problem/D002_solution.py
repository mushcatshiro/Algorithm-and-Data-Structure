...
# only works for [1, 2, 3, 4, 5], answer not generalized
...
class solution_4:
    ...
    def run(self):
        res, l, r = [1] * len(self.nums), 1, 1
        for i, j in zip(range(len(self.nums)), reversed(range(len(self.nums)))):
            res[i], l = res[i] * l, l * self.nums[i]
            res[j], r = res[j] * r, r * self.nums[j]
            # print(f'res: {res}, l: {l}, r: {r}, i: {i}, j: {j}')
        return res

# T O(n) S O(1)
# by far best solution, by adding two 1 at L, R of the list
# then track the accumulated multiplication
# to save a few keystrokes by replacing j as ~i